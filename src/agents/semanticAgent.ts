...
/**
 * Semantic Agent - Uses STANDARD tier (Nemotron) for high-volume tasks
 * Analyzes APIs, invariants, and design patterns.
 */
export const runSemanticAgent = async (chunks: CodeChunk[]): Promise<SemanticOutput> => {
  console.log('[Semantic] Using STANDARD tier (Nemotron) for semantic analysis...');
  const context = chunks.slice(0, 15).map(c => `Chunk: ${c.id}\n${c.content}`).join('\n---\n');
  return callGeminiAgent<SemanticOutput>(AGENT_PROMPTS.SEMANTIC, context, schema, 0.3, MODELS.STANDARD, ModelTier.STANDARD);
};