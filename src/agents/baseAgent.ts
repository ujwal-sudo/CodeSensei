...
export async function runAgent<T>(
  ...
  // Defer API key resolution to runtime when the agent is actually called
  ...
  // Try both constructor signatures (string or options object).
  let genAI: any;
  try {
    genAI = new GenAIConstructor(API_KEY as string);
  } catch (e) {
    genAI = new GenAIConstructor({ apiKey: API_KEY });
  }
